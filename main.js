...
document.getElementById("send-btn").addEventListener("click", () => {
    ...
    if (setupTextarea.value) {
      ...
      setupInputContainer.innerHTML = `<img src="/assets/images/loading.svg" class="loading" id="loading">`
...
async function fetchTitle(synopsis) {
  const response = await openai.createCompletion({
    model: "text-davinci-003",
    prompt: `Generate a short and alluring title for this synopsis: ${synopsis}
    ###
...
async function fetchStars(synopsis) {
  const response = await openai.createCompletion({
    model: "text-davinci-003",
    prompt: `Extract the actor names in backets from the synopsis: ${synopsis}
    ###
    actors: Ben Affleck, Natalie Portman
    ###
    actors: Evan Rachel Wood, Ed Harris, James Marsden
    ###
    actors: Tessa Thompson, Aaron Paul, Angela Sarafyan
    ###
    actors: 
    `,
    max_tokens: 30
